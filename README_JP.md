...
| フレーバー | バイナリ | WebUI | デーモン/CLI | Kasumi LKM | 最適な用途 |
|-----------|---------|-------|-------------|------------|-----------|
| **Full** | あり | あり | あり | あり | Kasumi の hide/spoof 機能を含む完全な機能セットが必要なユーザー。 |
...
`full` フレーバーには、3つのマウントバックエンド（OverlayFS、Magic Mount、Kasumi）、SolidJS WebUI、HTTP/SSE対応のUnixソケットデーモン、完全なCLI、Kasumi LKMアセットのすべてが含まれます。ほとんどのユーザーに推奨されるビルドです。
...
- 完全なグラフィカル管理体験を保ちつつ、ダウンロードサイズを抑えたい。

Liteビルドは `control-plane` フィーチャーセット（`kasumi` なし）を使用します。WebUIのKasumiパネルは自動的に非表示になります。

### Nano

`nano` フレーバーは**設定ファイル専用**のビルドです。WebUI、デーモン、CLI、すべての制御基盤を除外しています。残るのは、`config.toml` を読み込み、マウント計画を生成し、実行して終了する最小限のバイナリです。主な特徴：
...
- **デフォルトモードは `magic`** — Nanoは `default_mode = "magic"` がプリセットされており、ext4イメージを管理するデーモンなしで最大の互換性を得るためにバインドマウントを優先します。
- **モジュールモードマーカー** — インストール時の音量キー選択で、管理対象モジュールのルートに空の `overlay` または `magic` マーカーを置き、nano は白リストではなくそれを読み取ります。
- **ゼロランタイムオーバーヘッド** — 起動完了後、Hybrid Mountは実行中のプロセスを一切残しません。

予測可能でデーモンフリーのマウント統合を最小限のフットプリントで実現したい場合にNanoを選んでください。
...
リリースプロファイルは、最小限のバイナリサイズのために `opt-level = 3`、`lto = "fat"`、`codegen-units = 1`、`strip = true`、`panic = "abort"` を使用します。
